...
#include "neo_docking2/neo_charging_dock.hpp"

namespace neo_docking2
{

void NeoDock::configure(
  ...
  const std::string & name, std::shared_ptr<tf2_ros::Buffer> tf)
{
    ...
    nav2_util::declare_parameter_if_not_declared(
        node_, name + ".charging_threshold", rclcpp::ParameterValue(1));
    ...
    if (use_battery_status_) {
        ...
        "battery_state", 1,
        // define an anonymous lambda function that will be called when a new message arrives on /battery_state topic
        [this](const sensor_msgs::msg::BatteryState::SharedPtr state) {
            // if the current current is greater than the charging threshold, then the robot is charging
            // changed the above  to 
            // uint8 POWER_SUPPLY_STATUS_CHARGING=1
            is_charging_ = state->power_supply_status == charging_threshold_;
        });
    }
    ...
    if (use_stall_detection) {
        ...
        "joint_states", 1,
        std::bind(&NeoDock::jointStateCallback, this, std::placeholders::_1));
    }

    // create publishers
    dock_pose_pub_ = node_->create_publisher<geometry_msgs::msg::PoseStamped>("dock_pose", 1);
    filtered_dock_pose_pub_ = node_->create_publisher<geometry_msgs::msg::PoseStamped>("filtered_dock_pose", 1);
    staging_pose_pub_ = node_->create_publisher<geometry_msgs::msg::PoseStamped>("staging_pose", 1);

    // create publisher to publish dock pose as external feedback
    start_publishing_dock_pose_ = node_->create_publisher<std_msgs::msg::Bool>("start_publishing_dock_pose", 1);
}

geometry_msgs::msg::PoseStamped NeoDock::getStagingPose(
  const geometry_msgs::msg::Pose & pose, const std::string & frame)
{
    ...
    staging_pose.pose.position.z = 0.0;
    ...
    staging_pose_pub_->publish(staging_pose);
        // RCLCPP_INFO(node_->get_logger(), "________________________Staging pose: x=%f, y=%f, z=%f, orientation z=%f, w=%f in %s frame", staging_pose.pose.position.x, staging_pose.pose.position.y, staging_pose.pose.position.z, staging_pose.pose.orientation.z, staging_pose.pose.orientation.w, staging_pose.header.frame_id.c_str());
    return staging_pose;
}

bool NeoDock::getRefinedPose(geometry_msgs::msg::PoseStamped & pose)
    {
    ...
    // Publish true to trigger the pose publisher node
    auto message = std_msgs::msg::Bool();
    message.data = true;
    start_publishing_dock_pose_->publish(message);
...
bool NeoDock::isDocked()
{
  ...
  double d = std::hypot(
    ...
    bool are_we_close_enough = d < docking_threshold_;
    // Publish false to stop the pose publisher node since the robot is docked
  ...
  return are_we_close_enough;

}

// this should return true for charging to succeed
bool NeoDock::isCharging()
{   
  // Determine the charging status based on the condition
  return use_battery_status_ ? is_charging_ : isDocked();
}

//  This function is called before undocking commences to help prevent wear on the charge contacts.
bool NeoDock::disableCharging()
{   
  return true;
}

// This function is called while the controller is undocking
bool NeoDock::hasStoppedCharging()
{
  return !isCharging();
}

void NeoDock::jointStateCallback(const sensor_msgs::msg::JointState::SharedPtr state)
{
  double velocity = 0.0;
  double effort = 0.0;
...
PLUGINLIB_EXPORT_CLASS(neo_docking2::NeoDock, opennav_docking_core::ChargingDock)